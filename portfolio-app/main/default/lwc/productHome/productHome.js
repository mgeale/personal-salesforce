            ...
            this.products = data
                ...
                .map((item) => {
                    ...
                    const balance = item.Balances__r
                        ? this.formatterDecimal.format(
                              item.Balances__r[0].Balance__c
                          )
                        : null;
                    const balanceAUD = item.Balances__r
                        ? this.formatterCurrency.format(
                              item.Balances__r[0].Balance_AUD__c
                          )
                        : null;
                    return {
                        id: item.Id,
                        ...
                        priceChange,
                        priceStyle:
                            price - pricePrevious > 0 ? 'positive' : 'negative',
                        balance,
                        balanceAUD
                    };
                });
